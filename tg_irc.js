...
var irc_send_msg = function(irc_channel, msg) {
    console.log('  >> relaying to IRC: ' + msg);
    irc.say(irc_channel, msg);
};
...
var tg_send_msg = function(tg_chat_id, msg) {
...
irc.on('message', function(user, channel, message) {
    var conf = lookup('irc_channel', channel, config.channels);
    if (!conf) {
        return;
    }
    ...
    if (match || config.irc_relay_all) {
        ...
        if (!tg_chat_ids[conf.tg_chat]) {
            ...
            irc_send_msg(conf.irc_channel, err);
            console.log(err);
            return;
        }
        ...
        tg_send_msg(tg_chat_ids[conf.tg_chat], text);
    }
});
...
tg.on('message', function(msg) {
    ...
    irc_send_msg(conf.irc_channel, text);
});
